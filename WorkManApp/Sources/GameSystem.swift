...
class AchievementManager: ObservableObject {
    ...
    func claimDailyReward() -> (xp: Int, message: String)? {
        let today = dayFormatter.string(from: Date())
        ...
        // XP reward increases with streak (diminishing returns)
        let baseXP = 15
        let streakBonus = min(loginStreak * 5, 100)  // Max +100 at 20-day streak
        let totalXP = baseXP + streakBonus
        addXP(totalXP)

        // Streak milestone bonuses
        var message = "\(loginStreak)일 연속 출석! +\(totalXP) XP"
        if loginStreak == 7 {
            addXP(50)
            message = "🎉 7일 연속 출석! +\(totalXP + 50) XP 보너스!"
        } else if loginStreak == 14 {
            addXP(100)
            message = "🔥 14일 연속 출석! +\(totalXP + 100) XP 보너스!"
        } else if loginStreak == 30 {
            addXP(200)
            message = "👑 30일 연속 출석! +\(totalXP + 200) XP 대보너스!"
        } else if loginStreak == 100 {
            addXP(500)
            message = "💎 100일 연속 출석! +\(totalXP + 500) XP 전설 보너스!"
        } else if loginStreak == 365 {
            addXP(1000)
            message = "🌟 365일 연속 출석! +\(totalXP + 1000) XP 신화 보너스!"
        }

        saveState()
        return (totalXP, message)
    }
