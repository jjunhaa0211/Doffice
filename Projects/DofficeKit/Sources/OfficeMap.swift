...
public class OfficeMap: ObservableObject {
    public let cols: Int
    public let rows: Int
    public var tiles: [[TileType]]
    public var zones: [[OfficeZone?]]
    @Published public var furniture: [FurniturePlacement]
    ...
    public func isInBounds(_ c: TileCoord) -> Bool {
        c.col >= 0 && c.col < cols && c.row >= 0 && c.row < rows
    }

    public func isWalkable(_ c: TileCoord) -> Bool {
        guard isInBounds(c) else { return false }
        return _walkable[c.row][c.col]
    }

    public func tileAt(_ c: TileCoord) -> TileType {
        guard isInBounds(c) else { return .void }
        return tiles[c.row][c.col]
    }

    public func zoneAt(_ c: TileCoord) -> OfficeZone? {
    ...
    public func rebuildWalkability() {
    ...
    public func findPath(from start: TileCoord, to end: TileCoord) -> [TileCoord] {
    ...
    public static func defaultOffice(preset: OfficePreset = .cozy) -> OfficeMap {
        let cols = 42
        let rows = 20
    ...
    public static func defaultLayoutSnapshot(preset: OfficePreset = .cozy) -> OfficeLayoutSnapshot {
